...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    left.filterAcc(p).union(right.filterAcc(p, newAcc))
  }
  ...
  // OR right.union(left.union(that)).incl(elem)
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = matches(google)
  lazy val appleTweets: TweetSet = matches(apple)

  def matches(keywords: List[String]): TweetSet = TweetReader.allTweets.filter(
    tweet => keywords.exists(word => tweet.text.contains(word)))
