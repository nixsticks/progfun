...
trait Huffman extends HuffmanInterface {
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.map(freq => Leaf(freq._1, freq._2))
  ...
  def insertOrdered(tree: CodeTree, trees: List[CodeTree]): List[CodeTree] =
    trees.foldLeft(List[CodeTree]()) {
      (acc, curr) => if (weight(tree) < weight(curr)) tree :: curr :: acc else curr :: acc
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    @tailrec
    def loop(subtree: CodeTree, bits: List[Bit], acc: List[Char] = List()): List[Char] = subtree match {
      case l: Leaf => if (bits.isEmpty) l.char :: acc else  loop(tree, bits, l.char :: acc)
      case f: Fork => loop(if (bits.head == 0) f.left else f.right, bits.tail, acc)
    }

    loop(tree, bits).reverse
  }
...
//  For a given Huffman tree, one can obtain the encoded representation of a character by traversing from the root of the tree to the leaf containing the character. Along the way, when a left branch is chosen, a 0 is added to the representation, and when a right branch is chosen, 1 is added to the representation. Thus, for the Huffman tree above, the character D is encoded as 1011.
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    @tailrec
    def loop(subtree: CodeTree, text: List[Char], acc: List[Bit] = List()): List[Bit] =
      if (text.isEmpty) acc
      else subtree match {
        case _: Leaf => loop(tree, text.tail, acc)
        case f: Fork =>
          if (chars(f.left).contains(text.head)) loop(f.left, text, 0 :: acc) else loop(f.right, text, 1 :: acc)
      }

    loop(tree, text).reverse
  }
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.find(pair => pair._1 == char).get._2 // Sorry I'm just a lazy piece of garbage
